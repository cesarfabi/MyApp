...
sap.m.Text.prototype.setText=function(t){t=t||"";this.setProperty("text",t,true);var d=this.getDomRef();if(d){d.textContent=t}return this};
...
sap.m.Text.prototype._clampText=function(e){if(!this._bMutex){this._bMutex=true;this._deregisterResize();var i=(!e||!jQuery.browser.msie)?0:50;jQuery.sap.delayedCall(i,this,function(){if(!this.getDomRef()){this._bMutex=false;return}var n,a=0,t=this._$text[0],T=this.getText(),b=T.length,c=this.ellipsis.length,d=this._clampHeight();t.textContent=T;if(t.scrollHeight>d){while((b-a)>c){n=(a+b)>>1;t.textContent=T.slice(0,n-c)+this.ellipsis;if(t.scrollHeight>d){b=n}else{a=n}}if(t.scrollHeight>d&&a>0){t.textContent=T.slice(0,a-c)+this.ellipsis}}this._registerResize();this._bMutex=false})}};
sap.m.Text.prototype._clampHeight=function(){var l=parseFloat(this._$text.css("line-height")),l=l||parseFloat(this._$text.css("font-size"))*1.4,c=Math.floor(this.getMaxLines()*l);this._$text.css("max-height",c);return c};
