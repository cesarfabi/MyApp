...
sap.m.TablePersoDialog.prototype.init = function() {
	...
	this._columnItemTemplate = new sap.m.InputListItem({
		...
		content: new sap.m.Switch({state : "{Personalization>visible}"})
	});
	...
	this._oDialog = new sap.m.Dialog({
		...
		subHeader : new sap.m.Bar({
			...
			contentRight: new sap.m.Button({
				text: this._oRb.getText("PERSODIALOG_COLUMNS_ACTION_RESET"),
				press : function () {
					that._resetAll();
				}
			})
...
/**
* Sets the visibility for all columns to true in the model
* @private
*/
sap.m.TablePersoDialog.prototype._resetAll = function () {
	var oTable = sap.ui.getCore().byId(this.getPersoDialogFor());
	var oData = { aColumns : this._tableColumnInfo(oTable) };
	oData.aColumns = oData.aColumns.map(function (oColumn) {
